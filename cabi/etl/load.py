...
def load_counts(col):
    ...
    series = pd.read_sql(query, engine, parse_dates="time").set_index("time")
...
def load_counts_full():
    ...
    df = pd.read_sql(query, engine, parse_dates="time").set_index("time")
...
def load_trips():
    ...
    dfg = gpd.read_postgis(
        query, connection, geom_col="geom_coord", parse_dates="time"
    ).set_index("time")
...
def write_postgis(df, table_name="trips_long"):
    ...
    geodataframe["geom_coord"] = geodataframe["coord"].apply(
    ...
    geodataframe.to_sql(
        ...
        if_exists="append",  # if table exists append new values to the existing table
        ...
        dtype={
            "geom_coord": Geometry(
                geometry_type="POINT", srid=4326  # write as point objects
            )  # the spatial reference identifier for the coordinate system used in this set
        },
    )
